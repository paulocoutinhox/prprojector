...
def run_task_build(target):
    ...
    # qt tools
    qt_dir = os.path.join(current_dir, "build", "qt", c.qt_version, target["qt_dir"])
    ...
    command.append(
        os.path.join(build_dir, "PRProjector.app", "Contents", "MacOS", "PRProjector")
    )
    command.append("-o")
    ...
    command.append(
        os.path.join(
            pkg_dir,
            "PRProjector-macos-{0}-{1}.pkg".format(target["cpu"], target["mode"]),
        )
    )
